...
drop_and_leave()->
	...
	case bs:is_belief(gripper_open) of
		...
		_ -> 
			...
			update_and_communicate_count({drop_and_leave,true},bs:get_belief(othrAg))
...
get_to_drop()->
	...
	case {bs:is_belief(over_drop), bs:is_belief(seen),next_to(drop), executor:compare_value(robot, see, minor, 30),executor:compare_value(drop,see, mayor, 40)} of
		...
		{_,_,true,_,_} -> 
			...
			communicating_collect_bottles();
		{_,_,_,true,_} ->
			executor:execute([{get_to_drop,see_robot},send_message,{seen,self()}]),
			communicating_collect_bottles();
		{_,_,_,_,true}->
...
get_bottle()->
	...
	case {bs:is_belief(holding),executor:compare_value(touching, all, equal, centre) andalso bs:is_belief(gripper_open),bs:is_belief(touching) andalso bs:is_belief(gripper_open),bs:is_belief(seen),executor:compare_value(robot, see, minor, 30), next_to(bottle), bs:is_belief(see,bottle)} of
		...
		{_,_,_,true,_,_,_} ->
			...
			communicating_collect_bottles();	
		{_,_,_,_,true,_,_}->
			executor:execute([{get_bottle,see_robot},send_message,{seen,self()}]),
			communicating_collect_bottles();
		{_,_,_,_,_,true,_}->
...
send_message({_,Rule,Msg})->
	bs:update_belief({last_executed,{Rule,send,Msg}}),
...
%Implementaci�n de las acciones del tr
...
%Estas funciones me permiten modificar la BS mientras se est� ejecutando el tr, s�lo para testing, no formar�a parte del tr
